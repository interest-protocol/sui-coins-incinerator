...
const IncineratorTableInputMax: FC<IncineratorTableColumnAndRowProps> = ({
  ...
  const handleMax = (
    e: ReactMouseEvent<HTMLButtonElement> | ReactKeyboardEvent<HTMLInputElement>
  ) => {
    e?.stopPropagation();
...
const IncineratorTableInput: FC<IncineratorTableColumnAndRowProps> = ({
  ...
  const handleApprove = (
    e: ReactMouseEvent<HTMLDivElement> | ReactKeyboardEvent<HTMLInputElement>
  ) => {
    e?.stopPropagation();
  ...
  const onKeyDown = (e: ReactKeyboardEvent<HTMLInputElement>) => {
    if (e.code === Keys.Enter) return handleApprove(e);
    if (e.code === Keys.Escape) return handleCancel(e);
  };

  const handleDocumentKeyDown = (e: KeyboardEvent) =>
    onKeyDown(e as unknown as ReactKeyboardEvent<HTMLInputElement>);
